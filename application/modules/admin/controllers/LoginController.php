 ...
 *
 * Admin_LoginController  extends AdminCommonController. It is used to login to admin section.
 *
...
class Admin_LoginController  extends AdminCommonController
{
	...
	function indexAction()
	{
		...
		if($request->isPost()){
			...
			$authAdapter = new Zend_Auth_Adapter_DbTable($db); 
	   		...
	   		$authAdapter->setTableName('admin_master') 
			...
			if($this->view->loginError == '')
			{
				...
				{ 
				  ...
				  $records["last_login_date"] = date("Y-m-d H:i:s");
				  $auth = new Models_Login();
				  $result = $auth->UpdateAdminlogintime($records,$data->admin_id); 
				  
				  $mysession->Admin_Id = $data->admin_id;
				  $mysession->Admin_Login = $data->user_name;
					// Set cookie values.
				  	if($rembr == '1') 	{
						...
						setcookie ("cookie[admin_uname]", $cookie_username,$expire_time); 
						setcookie ("cookie[admin_passwd]", $cookie_password,$expire_time); 	
						setcookie ("cookie[admin_rembr]", $cookie_remember,$expire_time); 	
					}  else {
